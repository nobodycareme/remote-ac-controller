...
<#
.SYNOPSIS
    Builds every buildable artefact in the remote-ac-controller monorepo.

.DESCRIPTION
    Single entry point for a full local build.

    Scope, by design:
      * Firmware  : delegated to firmware/tools/dev.ps1 with the public profile.
                    This script never invokes pio.exe directly.
      * Cloud     : backend TypeScript compilation and frontend Vite build.

    Hard exclusions:
      * Never uploads, flashes or opens a serial port.
      * Never deploys to any server.
      * Never touches production credentials, certificates or the broker.
      * Never uses a non-public firmware profile.

.PARAMETER SkipFirmware
    Skip the firmware build (useful on a machine without PlatformIO Core).

.PARAMETER SkipCloud
    Skip the cloud build (useful on a machine without Node.js 24+).

.PARAMETER Clean
    Use dev.ps1 clean-build instead of build for the firmware.

.EXAMPLE
    pwsh tools/build-all.ps1

.EXAMPLE
    pwsh tools/build-all.ps1 -Clean
#>
[CmdletBinding()]
param(
    [switch] $SkipFirmware,
...
Write-Host ''
Write-Host 'remote-ac-controller :: build-all' -ForegroundColor White
Write-Host "repository : $RepoRoot"
Write-Host "profile    : $Profile (public only)"
Write-Host "mode       : $(if ($Clean) { 'clean-build' } else { 'build' })"
...
elseif (-not (Test-Path -LiteralPath $DevScript)) {
    ...
    $firmwareCommand = if ($Clean) { 'clean-build' } else { 'build' }
    Invoke-Step -Name "firmware :: dev.ps1 $firmwareCommand ($Profile)" -Action {
        & $DevScript -Command $firmwareCommand -Profile $Profile
        if ($LASTEXITCODE -ne 0) { throw "dev.ps1 $firmwareCommand exit code $LASTEXITCODE" }
    }
}
...
if ($SkipCloud) {
    ...
    if (-not $npm) {
        Add-Result -Name 'cloud' -Status 'SKIP' -Detail 'npm not on PATH'
        ...
        foreach ($pkg in @(
            ...
            Invoke-NodeStep -Name "cloud :: $($pkg.Name) build" `
                            -WorkingDirectory $pkg.Path `
...
Write-Host 'BUILD_ALL_RESULT=PASS' -ForegroundColor Green
exit 0
