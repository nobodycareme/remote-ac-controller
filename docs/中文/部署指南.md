...
使用以下命令生成 Owner 密码哈希，避免把明文写入命令历史：

```bash
node cloud/tools/generate-scrypt-hash.mjs
```

`WEB_USER` 与 `WEB_PASSWORD` 是唯一 Owner 登录凭据。旧的 `IR_OWNER_*`
环境变量会被忽略，不能启用真实红外。
...
cp ../shared/RemoteACCore/src/config/cloud_secrets.example.h ../shared/RemoteACCore/src/config/cloud_secrets.h   # MQTT 主机、账号、CA 或指纹
cp include/secrets.example.h       include/secrets.h         # 校园网账号（仅受控真实认证时需要）
```

在 `cloud_secrets.h` 中填写 MQTT 主机、端口、设备账号密码并嵌入 CA 证书；
`secrets.h` 仅在你确需 `ENABLE_CONTROLLED_LIVE_AUTH=1` 的受控真实校园认证时才创建。
这两个文件均被 git 忽略，一旦被提交 CI 会失败。

> Wi-Fi SSID **不在**这两个文件中配置：固件为 offline-first 设计，开机不自动联网，
> 由串口命令 `wifi connect <ssid>` 关联开放 SSID；校园网参数来自
> `CAMPUS_PROFILE_HEADER` 选定的 Profile 头文件。
...
5. **安全命令往返。** 在 IR 仍处于禁用状态下发一条命令；预期收到 `status=blocked_by_ir_policy` 与 `reason=real_ir_control_disabled`。这证明整条链路可用，同时证明物理红外仍被安全阻止。
