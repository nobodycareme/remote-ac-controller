**简体中文** | [English](../English/architecture.md)
...
**红外派发只有一个入口** —— `src/cloud/command_service.cpp` 中的 `dispatchIrAction()`。真实 IR 关闭时，`set_power` / `set_temperature` 命令绝不会发射红外，而是以 `blocked_by_ir_policy` 状态及 `real_ir_control_disabled` 原因确认应答。这保证了每一次真实红外发射都必须经过唯一的策略检查点。
...
- **内嵌数据库。** `node:sqlite`（Node 22+ 内置）省去了本地原生编译步骤，这对小型服务器意义重大。参见 [`resource-constrained-deployment.md`](./低配置服务器部署.md)。
...
- 没有原生数据库驱动，没有 ORM。
- 不包含任何生产凭据、TLS 密钥、红外帧数据或数据库。参见 [`security-model.md`](./安全模型.md)。
