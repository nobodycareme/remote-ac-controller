...
以 [`cloud/deploy/secrets.env.example`](../../cloud/deploy/secrets.env.example) 为起点。必需的条目包括两个 MQTT 账号（后端的 `MQTT_USERNAME` / `MQTT_PASSWORD`，设备的 `MQTT_DEVICE_USERNAME` / `MQTT_DEVICE_PASSWORD`）、`SESSION_SECRET`、`WEB_USER` 与 `WEB_PASSWORD`。

卫生规则：

- 设为 `chmod 600` 且由 root（或服务用户）拥有。它仅在启动时读取。
- 所有者密码以 scrypt 摘要形式存储，格式为 `salt:hash`，绝不以明文保存。见 [`security-model.md`](./安全模型.md)。
- **轮换 `SESSION_SECRET` 或 `WEB_PASSWORD` 会使所有现有会话失效**，包括受信任设备。这是有意为之，也是驱逐所有客户端最快的方式。
...
随后确认端到端行为：登录，观察新鲜遥测数据，并在真实 IR 仍禁用的情况下发送一条无害命令——确认应答（ACK）应报告 `blocked_by_ir_policy`，原因为 `real_ir_control_disabled`。
...
按顺序检查：真实 IR 安全总开关（§7）——若已禁用，确认应答（ACK）按设计返回 `blocked_by_ir_policy` / `real_ir_control_disabled` 且不发射任何 IR；然后是 IR 码注册；再是发射器朝向与供电。见 [`ir-learning.md`](./红外学习.md)。
