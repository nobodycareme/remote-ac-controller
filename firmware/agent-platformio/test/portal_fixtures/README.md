...
所有内容均为公开信息（门户主机 `w.xidian.edu.cn` 为西安电子科技大学公开域名），**不含**任何账号、密码、
MAC、Token 或个人数据。

| 文件 | 场景 | 期望 classify | 期望 AC_ID |
|------|------|---------------|-----------|
| `01_3xx_redirect.http` | 3xx 重定向到门户网管（Location 含 ac_id） | captive=YES | 8 |
| `02_200_autosubmit_form.html` | 200 透明拦截，含自动提交 form（action 指向 `w.xidian.edu.cn/srun_portal.php`；命中的是与 Profile 无关的 `srun_portal` 标记） | captive=YES | 8 |
| `02b_200_metarefresh_srunportal.html` | 200 透明拦截，含 meta-refresh 到 `srun_portal_pc?ac_id=8`（**真实校园形态**，body 中完全没有门户主机名字面量） | captive=YES | 8 |
