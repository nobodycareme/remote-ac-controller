...
const stmts = {
  insertTelemetry: null as any,
  upsertState: null as any,
  insertCommand: null as any,
  markPublished: null as any,
  updateAck: null as any,
  insertEvent: null as any,
  setWeather: null as any,
};

export function prepareStatements(): void {
  stmts.insertTelemetry = db.prepare(`INSERT INTO telemetry
    ...
    VALUES (@device_id, @seq, @server_received_at, @temperature_c, @humidity_pct, @sensor_ok, @wifi_rssi_dbm, @free_heap_bytes, @uptime_s, @wifi_reconnect_count, @mqtt_reconnect_count, @firmware_version, @simulated)`);

  stmts.upsertState = db.prepare(`INSERT INTO device_state
    ...
    ON CONFLICT(device_id) DO UPDATE SET
      ...
      control_mode=excluded.control_mode, updated_at=excluded.updated_at, simulated=excluded.simulated`);

  stmts.insertCommand = db.prepare(`INSERT OR IGNORE INTO commands
    (command_id, device_id, action, requested_power, requested_temperature_c, status, created_at, expires_at, failure_reason)
    VALUES (@command_id, @device_id, @action, @requested_power, @requested_temperature_c, @status, @created_at, @expires_at, @failure_reason)`);

  stmts.markPublished = db.prepare(`UPDATE commands SET status='published', published_at=@published_at WHERE command_id=@command_id`);
  stmts.updateAck = db.prepare(`UPDATE commands SET status=@status, acknowledged_at=@acknowledged_at, failure_reason=@reason WHERE command_id=@command_id`);
  stmts.insertEvent = db.prepare(`INSERT INTO events (event_type, device_id, message, created_at) VALUES (@event_type, @device_id, @message, @created_at)`);
  stmts.setWeather = db.prepare(`INSERT INTO weather_cache (location, payload_json, observed_at, fetched_at, expires_at)
    VALUES (@location, @payload_json, @observed_at, @fetched_at, @expires_at)
    ON CONFLICT(location) DO UPDATE SET payload_json=excluded.payload_json, observed_at=excluded.observed_at, fetched_at=excluded.fetched_at, expires_at=excluded.expires_at`);
}

export function insertTelemetry(r: TelemetryRow): void {
  stmts.insertTelemetry.run(r);
}
...
export function markCommandPublished(command_id: string, published_at: number): void {
  stmts.markPublished.run({ command_id, published_at });
}

export function updateCommandAck(command_id: string, status: string, acknowledged_at: number, reason?: string): void {
  stmts.updateAck.run({ command_id, status, acknowledged_at, reason: reason ?? null });
}

export function insertEvent(event_type: string, device_id: string, message: string): void {
  stmts.insertEvent.run({ event_type, device_id, message, created_at: Date.now() });
}
...
export function setWeatherCache(location: string, payload_json: string, observed_at: number, fetched_at: number, expires_at: number): void {
  stmts.setWeather.run({ location, payload_json, observed_at, fetched_at, expires_at });
}

export function getLatestTelemetry(): any {
  return db.prepare(`SELECT * FROM telemetry WHERE device_id=? ORDER BY server_received_at DESC LIMIT 1`).get(config.DEVICE_ID);
}
export function getDeviceState(): any {
  return db.prepare(`SELECT * FROM device_state WHERE device_id=?`).get(config.DEVICE_ID);
}
export function getRecentCommands(limit = 20): any[] {
  return db.prepare(`SELECT * FROM commands WHERE device_id=? ORDER BY created_at DESC LIMIT ?`).all(config.DEVICE_ID, limit);
}
export function getCommand(command_id: string): any {
  return db.prepare(`SELECT * FROM commands WHERE command_id=?`).get(command_id);
}
...
export function getWeatherCache(location: string): any {
  return db.prepare(`SELECT * FROM weather_cache WHERE location=?`).get(location);
}
...
export function getTelemetryHistory(rangeMs: number, maxPoints = 240): { t: number; temperature_c: number; humidity_pct: number }[] {
  ...
  // 1-minute SQL aggregation buckets (provides 1-min granularity on demand)
  const rows = db.prepare(`SELECT (server_received_at/60000)*60000 AS t,
...
// Retention: raw telemetry 7d; commands/events 180d. Runs periodically.
export function retentionCleanup(): { deletedTelemetry: number; deletedEvents: number } {
  ...
  const d1 = db.prepare(`DELETE FROM telemetry WHERE server_received_at < ?`).run(telCut);
  const d2 = db.prepare(`DELETE FROM events WHERE created_at < ?`).run(evCut);
  db.prepare(`DELETE FROM commands WHERE created_at < ?`).run(evCut);
